...
const ID_A = "deckard@thekysek.github.io";
const ID_B = "{01234567-89ab-cdef-0123-456789abcdef}";
...
test("installation is idempotent, conflict-safe, relocatable, and independent of Python", { skip: !available }, t => {
  ...
  const args = ["install", "--home", home, "--manifest-dir", manifests, "--extension-id", ID_A,
  ...
  child = run(binary, args.map(value => value === ID_A ? ID_B : value));
  ...
  assert.deepEqual(manifest.allowed_extensions, [ID_A]);
  ...
  // Firefox launches the host with the manifest path and the calling add-on's ID.
  child = run(manifest.path, [manifestPath, ID_A], Buffer.concat([prefix, message]));
  assert.equal(child.status, 0, child.stderr.toString());
  assert.equal(JSON.parse(child.stdout.subarray(4)).result.model_loaded, false);
  for (const caller of [[manifestPath, ID_B], [manifestPath], [`chrome-extension://${ID_A}/`]]) {
    child = run(manifest.path, caller, Buffer.concat([prefix, message]));
    assert.equal(child.status, 1);
    assert.match(child.stderr.toString(), /origin_mismatch/);
  }
  ...
  child = run("ldd", [installed]);
  assert.equal(child.status, 0, child.stderr);
  assert.doesNotMatch(child.stdout, /not found|\.venv|libpython|libmlx|libcurl/);
  // $ORIGIN resolves beside the invoked executable: the release's own ONNX Runtime copy.
  assert.ok(child.stdout.includes(`libonnxruntime.so.1 => ${path.join(relocated, "current/bin")}/../lib/libonnxruntime.so.1`),
    child.stdout);
  assert.ok(!files.some(file => /packed\.safetensors|libmlx|mlx\.metallib|mlpackage/.test(file)));
...
test("registration failure cannot activate a new release", { skip: !available }, t => {
  ...
  const args = ["install", "--home", home, "--manifest-dir", manifests, "--model-dir", model,
    "--extension-id", ID_A, "--shell", "none", "--no-extension"];
  ...
  child = run(binary, [...args.map(value => value === ID_A ? ID_B : value), "--replace"]);
  assert.equal(child.status, 1);
